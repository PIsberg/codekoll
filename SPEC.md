...
| `api-misuse` | Standard-library contracts violated | 13 |
...
| `CK-ITERATOR-DOUBLE-NEXT` | W | A `while`/`do`/`for` loop whose condition calls `it.hasNext()` (receiver resolving to a `java.util.Iterator` variable) and whose body calls `it.next()` on that same variable two or more times on one path — the guard promises one element, the body consumes two, so an odd number of remaining elements throws `NoSuchElementException`. *(Not descended into: nested loops, lambdas, local classes, `switch`, ternaries, and `if`s whose condition re-checks `hasNext()` on that iterator — each may legitimately re-guard. Calls in opposite branches of one `if` are exclusive and not flagged. WARNING rather than ERROR because deliberate pairwise consumption of a known-even stream is a real, if fragile, shape.)* |
...
| `CK-REGEX-GROUP-INDEX` | E | A capturing group the pattern does not have. Counts capturing groups in a compile-time-constant regex (plain `(` and named `(?<name>`, skipping escapes, character classes, and the non-capturing `(?:` / `(?=` / `(?<=` forms), then flags (a) `matcher.group(n)` with `n` > count, where the matcher is traceable to `Pattern.compile(literal)` through a variable or an inline chain, and (b) a `$n` reference in a constant `replaceAll`/`replaceFirst` replacement whose leading digit exceeds the count — matching `Matcher`'s own digit-consuming parse. Both throw `IndexOutOfBoundsException: No group n` on the first execution. *(Non-constant patterns, matchers with no traceable pattern, `group(String)`, escaped `\\$`, and in-range indices are all exempt.)* |
| `CK-PRIMITIVE-ARRAY-VARARGS` | W | A single argument of **primitive array** type passed to `Arrays.asList` or `Stream.of` (receiver resolving to `java.util.Arrays` / `java.util.stream.Stream`). Varargs spreading needs a reference array, so `T` is inferred as `int[]` and the result holds the array as **one** element: `size() == 1`, `contains(x)` always false, `toString` prints `[I@…`. Nothing throws. Suggest `Arrays.stream(a).boxed().toList()` / `Arrays.stream(a)`. *(Exempt reference arrays, which spread correctly; exempt a call with several arguments (genuine varargs); exempt an explicitly written declaration whose type argument is an array — `List<int[]> rows = Arrays.asList(row);` — but not the same thing inferred through `var`, which is the bug rather than a statement of intent.)* |
| `CK-IMMUTABLE-FACTORY-DUPLICATE` | E | The same statically-known value twice among `Set.of` elements, `Map.of` keys (even argument positions), or `Map.ofEntries(Map.entry(k, v), …)` keys. Value identity = literal/constant-field value keyed by its runtime type, or an enum constant's qualified name. Both factories reject duplicates → guaranteed `IllegalArgumentException: duplicate element/key`, typically as `ExceptionInInitializerError` from a `static final` table. *(`List.of` permits duplicates and is never flagged; non-constant arguments that might coincide are not flagged; `HashSet`/`HashMap` are exempt by contract — they legitimately keep the last value.)* |
