...
<!-- Badge row — wired to the named CI jobs in Milestone 10 (see PLAN.md):
     build · tests+examples · coverage · checkstyle · pmd · spotbugs · errorprone/nullaway (jspecify) · archunit · loadtest -->
...
1. **Every finding is worth reading.** A curated catalog of 106 rules, each with an explicit false-positive budget: exemptions are part of every rule's contract and covered by tests, heuristic rules are demoted to INFO and never break your build, and any false positive found in the wild becomes a regression test before it's fixed. The opposite philosophy of "400 patterns your team disables wholesale."
...
3. **Rules that teach.** Every rule carries *what is wrong*, *what happens at runtime*, and *how to fix it* as first-class metadata — shown in findings, via `codekoll --explain CK-…`, and in a runnable examples project with a documented buggy/fixed pair for all 106 rules. Compare that to decoding `RV_RETURN_VALUE_IGNORED`.
...
106 rules in ten packs — all bugs that **compile cleanly and fail (or silently misbehave) at runtime**:

| Pack | Focus | Rules | Flavor |
|---|---|---|---|
| `correctness` | Provably or almost-certainly wrong logic | 26 | discarded `String.trim()`, `==` on Strings, `x.equals(null)`, the `YYYY` week-year date bug, format-string mismatches, infinite recursion, `getBytes()` without a charset, arrays as hash keys |
...
| `concurrency` | Threading and synchronization | 13 | `Thread.run()` instead of `start()`, `synchronized` on a `Lock`, `++` on volatile, double-checked locking without `volatile`, parallel streams mutating an `ArrayList`, discarded `CompletableFuture`s |
...
| `api-misuse` | Stdlib contracts violated | 8 | `Map<String,User>.get(12345)` (always null), mutating `List.of()`, `(String[]) list.toArray()`, `split(".")`, `List<Integer>.remove(1)` (index, not value!), `toMap` without a merge function |
...
The full catalog — every rule's detection algorithm and exemption list — lives in [SPEC.md](SPEC.md). Each rule also has a runnable, documented example in [`codekoll-examples`](codekoll-examples/) showing the bug, the runtime failure, and the fix.
...
**The 30-second demo:** run codekoll on its own examples module and watch all 106 rules fire —
