...
![examples](https://img.shields.io/badge/examples-113%2F113-brightgreen)
...
![rules](https://img.shields.io/badge/rules-113-blue)
...
1. **Every finding is worth reading.** A curated catalog of 113 rules, each with an explicit false-positive budget: exemptions are part of every rule's contract and covered by tests, heuristic rules are demoted to INFO and never break your build, and any false positive found in the wild becomes a regression test before it's fixed. The opposite philosophy of "400 patterns your team disables wholesale."
...
3. **Rules that teach.** Every rule carries *what is wrong*, *what happens at runtime*, and *how to fix it* as first-class metadata — shown in findings, via `codekoll --explain CK-…`, and in a runnable examples project with a documented buggy/fixed pair for all 113 rules. Compare that to decoding `RV_RETURN_VALUE_IGNORED`.
...
113 rules in eleven packs. Ten target bugs that **compile cleanly and fail (or silently misbehave) at runtime**; the eleventh, `pqc`, inventories cryptography that works today and a quantum computer breaks:
...
| `concurrency` | Threading and synchronization | 12 | `counter.set(counter.get() + 1)` on an Atomic (a lost update), `Thread.run()` instead of `start()`, `synchronized` on a `Lock`, `++` on volatile, double-checked locking without `volatile`, collection modified while iterating, discarded `CompletableFuture`s |
...
| `pqc` | Quantum-vulnerable asymmetric crypto; warns, never fails a default build | 3 | RSA encryption, RSA/ECDSA/EdDSA signatures, (EC)DH and X25519, `DHKEM`, classical TLS groups and suites, XML signature methods; every name, alias and OID the JDK defines, with ML-KEM/ML-DSA guidance for the project's Java release |
...
**The 30-second demo:** run codekoll on its own examples module and watch all 113 rules fire —
