...
- T043: 11 issues opened, #63 to #73: the 9 research follow-ups (#63 CK-CRYPTO-WEAK on RSA/ECB, reproduced with the built jar and failing a default build), plus #68 (cipher suites via system properties, analysis finding G1) and #73 (SC-006 precision review, analysis finding G4). #75 (load-test heap gate measures the Maven JVM) added after CI, see T044.
- T044: PR #74, not merged. CI: every job green except `loadtest`, whose heap gate failed on 3bdf373 (126 MB, +22.9%; re-run 126 MB, +22.3%), passed once on 447590e (115 MB) and failed again on 8dddfcf with identical code (126 MB, +22.8%). The 447590e hypothesis (platform lookup in `builtInPqc`) is refuted by that last run. Confirmed cause: the harness runs via `exec:java` in the Maven JVM and measures heap retained by Checkstyle/PMD/SpotBugs from the same invocation; locally the same command gave 87 and 75 MB with those plugins, 61 and 61 MB without (+2.1%). Separate-JVM probe: the pack retains 24 KB of 12.7 MB. Gate left red rather than raising thresholds or refreshing `baseline.json`; tracked in #75 for a harness fix in its own PR.
