...
- [ ] T043 Open one GitHub issue per follow-up in `specs/001-pqc-migration-scanner/research.md` "Follow-ups" (8 issues) with `gh issue create`, each stating what is missing, why it was not done here, and what it would take; collect the issue numbers
- [ ] T044 Push `001-pqc-migration-scanner`, open a PR to `main` with `gh pr create` whose body states the failure prevented (quantum-vulnerable crypto with no inventory), how it was verified (T020, T030, T032 outputs, T041 numbers, JDKs run in T033), the Milestone 15 baseline dependency, and links to the T043 issues; do not merge. Follow the CI run to green on every job (`build`, `quality`, `coverage`, `archunit`, `examples`, `selfcheck`, `loadtest`), fixing and pushing until it is green, and report the run's actual state
